...
game2.filter('toArray', function() { return function(obj) {
 ...
 *******************************************************************/
game2.service('database', function(Restangular, $state, aliens) {
 ...
 *******************************************************************/
game2.service('update2',function(helper, bucket2, aliens, style) {
  ...
  this.updateIllegalAlien = function() {
    ...
    for (var i = 0; i < bucket2.buckets[bucket2.current_bucket].alien.length; i++) {
      var model_num = helper.get_model(bucket2.buckets[bucket2.current_bucket].alien[i]);
    ...
    for (var id in aliens.alienArray) {
      ...
      if (models_in_bucket.indexOf(model_num) != -1 && bucket2.buckets[bucket2.current_bucket].alien.indexOf(id) == -1) {
 ...
 *******************************************************************/
game2.service('style', function(aliens, helper) {
  ...
  this.highLight = function(alien_id, bucket2, method_flag) {
    // Get all aliens in current bucket.
    var members = bucket2.alien;
 ...
 *******************************************************************/
game2.service('bucket2', function(style, $timeout, aliens, history) {
  ...
  this.orderAlienArray = function() {
    this.orderedIds = [];
    this.parentOrderedIds = [];

    for (var i = 0; i < this.buckets.length; i++) {
      this.parentOrderedIds.push(this.buckets[i].alien);
    }
    ...
    this.parentOrderedIds.push(this.orderedIds);
...
game2.service('history', function(aliens) {
